...
foreach ($extraFields as $extrafield) {
	...
	if ($extrafield->alias == 'author') $xAuthor = $extrafield->value;
	...
	<div class="g-grid">
		<div class="g-block size-35">
			<div class="g-content">
				...
				<!-- Item title -->
				<h3 class="item-title uk-margin-large-top uk-animation-slide-left">
				...
				<?php if($this->item->params->get('catItemDateCreated')) { ?>
					<!-- Date created -->
					<div class="uk-article-meta date uk-margin-large-bottom">
			...
			<?php if($this->item->params->get('itemImage')) { ?>
				<div class="item-image uk-animation-slide-bottom">
				<?php if((!empty($this->item->image)) || (!empty($xIntroImage)) || (!empty($xFullImage))) { ?>
					<figure class="uk-overlay">
						<?php
						if(!empty($xFullImage)) { ?>
							<a href="<?php echo $xFullImage; ?>" alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" data-uk-lightbox><img src=<?php echo $xFullImage; ?> alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>"/></a>
						<?php
						} elseif (!empty($this->item->image)) { ?>
							<a href="<?php echo $this->item->image; ?>" alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" data-uk-lightbox><img src="<?php echo $this->item->image; ?>" alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>"/></a>
						<?php
						} else { ?>
							<a href="<?php echo $xIntroImage; ?>" alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>" data-uk-lightbox><img src=<?php echo $xIntroImage; ?> width="100%"; height="auto" alt="<?php if(!empty($this->item->image_caption)) echo K2HelperUtilities::cleanHtml($this->item->image_caption); else echo K2HelperUtilities::cleanHtml($this->item->title); ?>"/></a>
						<?php } ?>
						<?php if(!empty($xQuote)) {?>
		...
		<div class="g-block size-65 uk-flex-middle uk-flex-column item-content">
			<div class="g-content uk-animation-slide-right">
